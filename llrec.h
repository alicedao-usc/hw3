...
template <typename Comp>
Node* helper(Node* head, Comp pred, Node* prev, Node* trueHead)
{
    if (head == nullptr)
    {
        return trueHead;
    ...
    if(pred(head->val))
    {
        if (trueHead == head)
        {
            trueHead = head -> next;
            delete head;
            return helper(trueHead, pred, nullptr, trueHead);
        }
        else
        {
            prev -> next = head ->next;
            Node* temp = head ->next;
            delete head;
            return helper(temp,pred,prev,trueHead);
        }
    }
    
    else
    {
        return helper(head->next, pred, head, trueHead);
...
Node* llfilter(Node* head, Comp pred)
{
    ...
    return helper(head, pred, nullptr, head);
    //return head;
